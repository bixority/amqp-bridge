...
impl Config {
    ...
    {
        Ok(Self {
            source_dsn: get_var("SOURCE_DSN").ok_or_else(|| {
                BridgeError::Config("SOURCE_DSN environment variable not set".to_string())
            })?,
            source_queue: get_var("SOURCE_QUEUE").unwrap_or_else(|| "old".to_string()),
            target_dsn: get_var("TARGET_DSN").ok_or_else(|| {
                BridgeError::Config("TARGET_DSN environment variable not set".to_string())
            })?,
